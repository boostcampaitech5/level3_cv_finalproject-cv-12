...
class DressCodeDataset(data.Dataset):
    def __init__(self,
        ...
        for c in category:
            ...
            dataroot = self.dataroot ## 수정
            if phase == 'train':
                filename = os.path.join(dataroot, 'input/buffer', f"{phase}_pairs.txt")
            else:
                filename = os.path.join(dataroot, 'input/buffer', f"{phase}_pairs_{order}.txt")
